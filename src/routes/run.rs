    ...
    .await{
        ...
        Err(e) => {
            ...
            match e {
                ...
                BlockingError::Error(diesel::NotFound) => Err(HttpResponse::NotFound().json(ErrorBody {
...
fn get_bytes_for_csv_zip_from_runs(runs: &Vec<RunWithResultsAndErrorsData>) -> Result<Vec<u8>, HttpResponse> {
    // Build csv files from results
    let run_csvs_dir: TempDir = match run_csv::write_run_data_to_csvs_and_zip_in_temp_dir(&runs) {
        ...
        Err(e) => {
            ...
            debug!("Encountered error while trying to generate csvs for runs {:#?} : {}", runs, e);
            return Err(HttpResponse::InternalServerError().json(ErrorBody{
                ...
                status: 500,
                detail: format!("Encountered error while trying to generate csv files for retrieved run(s): {}", e)
            }))
        }
    };
    ...
    match fs::read(zip_file_path) {
        ...
        Err(e) => {
            ...
            debug!("Encountered error while trying to read csv zip for runs {:#?} : {}", runs, e);
...
mod tests {
    ...
    fn get_csv_zip_bytes_for_test_runs(runs: &Vec<RunWithResultsAndErrorsData>) -> Vec<u8> {
        let run_csvs_dir: TempDir = run_csv::write_run_data_to_csvs_and_zip_in_temp_dir(&runs).unwrap();
        // Get zip of csvs as bytes to return as response body
        let mut zip_file_path: PathBuf = PathBuf::from(run_csvs_dir.path());
        zip_file_path.push("run_csvs.zip");
    ...
    async fn find_for_template_success_csv() {
        ...
        let req = test::TestRequest::get()
            .uri(&format!("/templates/{}/runs?csv=true", new_test.template_id))
            .to_request();
        let resp = test::call_service(&mut app, req).await;
